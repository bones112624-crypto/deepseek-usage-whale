...
| 本轮缓存命中率 | 「命中输入 ÷（命中 + 未命中）」，等于命中输入 ÷ 全部输入 token。直接读 Codex 自己算好的 `payload.turn_token_usage`（拿不到才退回逐条求和），所以和 Codex 的口径逐轮一致，一轮进行中还会跟着日志增长动态刷新，不会冻结在第一次看到的值。缺数据时显示 `本轮命中 —` 而不是 0%；cached 超过 input 时按 input 截断，永远不会超过 100%。同时开着多个会话时，优先认 `~/.codex/thread-writer-locks/<threadId>.lock` 指明的「正在写的那个会话」，读不到锁才退回按时间戳挑最新。99% 以上保留一位小数，免得四舍五入成 100% |
...
改成 `false` 之后既不自动放出、也不会被自动收掉。用桌面快捷方式或 `whale.mjs start`
手动放出时，如果 Codex 当时没开着就一直留着；一旦见 Codex 运行过，Codex 退出时它也会跟着收起。
另外用 `Ctrl+Shift+Z` 主动关掉之后，同一个 Codex 会话里不会再被自动弹回来，Codex 重启后恢复。
