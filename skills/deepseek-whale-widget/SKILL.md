...
- **对话气泡**：代码绘制（大椭圆 + 尾巴 + 两个小气泡），描边 `#203170`，白底，内部四行文字
  ...
  - 缓存行 `上轮命中 N%`（48u，颜色 `#9fb0d9`），取自最近一轮对话的真实 usage，99% 以上保留一位小数。
    多个 Codex 会话同时开着时，按「最后一条 usage 记录的时间戳」取真正最新的那一轮，不按文件修改时间。
    每轮消耗气泡的第三行显示的是**那一轮跑完后的完整值**（新的一轮开始时才统计上一轮）。
    注意它与用量监控面板上的「当日缓存命中率」口径不同：这里只算一轮，长会话里天然接近 100%
